...
class ModelSignalHandler(SignalHandler):
    ...
    def init(meas_controller):
        ...
        meas_controller.model.equipment_changed.connect(
            lambda message: EquipmentSignalHandler.handler(meas_controller, message)
        )
        meas_controller.model.settings_changed.connect(
            lambda message: SettingsSignalHandler.handler(meas_controller, message)
        )
        meas_controller.model.progress_status.connect(
            lambda message: ProgressSignalHandler.handler(meas_controller, message)
        )
        meas_controller.model.s21_file_changed.connect(
