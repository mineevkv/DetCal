...
class OscilloscopeSheet(InstrumentSheet):
    """
    Class for oscilloscope sheet in the main window.

    This class is used to create the oscilloscope sheet in the main window.
    It contains controls for vertical and horizontal scale and position,
    channel buttons, run/stop and single sequence buttons, trigger controls.
    ...
    def __init__(self, main_layout) -> None:
        ...
        self.add_control_elem(
            "VERT_SCALE", self.zero_col, scale_row, "Scale, mV:", "20", *args
        )
        self.add_control_elem(
            "VERT_POS", self.zero_col, pos_row, "Offset, mV:", "0", *args
        )
        ...
        self.add_label("VERT", 7, self.ip_row + 1, "VERTICAL").setProperty(
            "class", "osc_label"
        )
        self.add_label("HOR", hor_col + 7, self.ip_row + 1, "HORISONTAL").setProperty(
            "class", "osc_label"
        )

        self.add_frame("CH1", self.zero_col, ch_row, 4, 4, 42, 29, "ch1_frame")
        self.add_btn("CH1", self.zero_col, ch_row, "CH1", ch_btn_width).setProperty(
            "class", "btn_ch1"
        )
        ...
        self.add_btn(
            "CH2", self.zero_col + dx, ch_row, "CH2", ch_btn_width
        ).setProperty("class", "btn_ch2")
        self.elem["BTN_CH2"].setCheckable(True)

        self.add_frame("CH3", self.zero_col + 2 * dx, ch_row, 4, 4, 42, 29, "ch3_frame")
        self.add_btn(
            "CH3", self.zero_col + 2 * dx, ch_row, "CH3", ch_btn_width
        ).setProperty("class", "btn_ch3")
        self.elem["BTN_CH3"].setCheckable(True)

        self.add_frame("CH4", self.zero_col + 3 * dx, ch_row, 4, 4, 42, 29, "ch4_frame")
        self.add_btn(
            "CH4", self.zero_col + 3 * dx, ch_row, "CH4", ch_btn_width
        ).setProperty("class", "btn_ch4")
        self.elem["BTN_CH4"].setCheckable(True)

        self.add_btn(
            "HI_RES", self.zero_col + 4 * dx, ch_row, "HiRes", 50
        ).setCheckable(True)

        param = ch_row - 1, "MEG", ch_btn_width
        self.add_label("CH1_IMP", self.zero_col, *param).setProperty(
            "class", "osc_imp_label"
        )
        self.add_label("CH2_IMP", self.zero_col + dx, *param).setProperty(
            "class", "osc_imp_label"
        )
        self.add_label("CH3_IMP", self.zero_col + 2 * dx, *param).setProperty(
            "class", "osc_imp_label"
        )
        self.add_label("CH4_IMP", self.zero_col + 3 * dx, *param).setProperty(
            "class", "osc_imp_label"
        )

        self.add_btn("RUN", 24, self.zero_row, "Run/Stop").setCheckable(True)
        self.add_btn("SINGLE", 31, self.zero_row, "Single").setProperty(
            "class", "btn_singleseq"
        )
