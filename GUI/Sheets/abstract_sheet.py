...
class Sheet(ABC):
    def __init__(self, main_window):
        ...
        self.x_col = [
            x
            for x in range(
                self.margin_left, main_window.width() - self.margin_left, self.col_width
            )
        ]
        self.y_row = [
            y
            for y in range(
                self.margin_top, main_window.height() - self.margin_top, self.row_hight
            )
        ]
    ...
    def add_label(self, key, col, row, text, width=85):
        ...
        label.setGeometry(
            QtCore.QRect(self.x_col[col], self.y_row[row], width, self.elem_hight)
        )
        return label
    ...
    def add_progress_bar(self, key, col, row, width, hight):
        ...
        progress_bar.setGeometry(
            QtCore.QRect(self.x_col[col], self.y_row[row], width, hight)
        )
