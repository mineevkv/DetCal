...
class VisaCom:
    """
    Basic class for VISA communication
    """
    ...
    def send(self, command: str) -> str | None:
        ...
        Returns:
            Response string for queries, bytes written for commands, None on error
        """
        ...
        try:
            ...
            logger.info(
                f"{self.instr} sending {command_type} command >> {command} -> {response}"
            )
            return response
    ...
    @staticmethod
    def get_visa_string_ip(ip: str) -> str:
        """
    ...
    def get_visa_resource(
        visa_string: str, alternative: bool = False
                ...
                if alternative:
                    ...
                    for read_term, write_term in [
                        ...
                        try:
                            ...
                            idn = inst.query("*IDN?").strip()
                            logger.info(
                                f"Success with termination {repr(read_term)}/{repr(write_term)}: {idn}"
                            )
                            return inst

                        except pyvisa.errors.VisaIOError:
                            logger.error(
                                f"All termination character combinations failed for {interface}"
                            )
                            raise
